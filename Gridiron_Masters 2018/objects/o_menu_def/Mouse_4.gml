...
if position_meeting(mouse_x, mouse_y, self) {
	if instance_exists(o_menu_item_def) {
		with (o_menu_item_def) {
			instance_destroy();
		}
	}
	else {
		...
		for (var i = 0; i < ds_list_size(my_list); i++) {
			var item = instance_create_depth(x, yy, 0, o_menu_item_def);
	...
	if !position_meeting(mouse_x, mouse_y, o_menu_item_def) {	
			if instance_exists(o_menu_item_def) {
				with (o_menu_item_def) {
					instance_destroy();
				}
